...
pub struct PushedAuthorizationClient<C, JT, JE, JA>
where
    C: Profile,
    JT: JsonWebKeyType,
...
impl<C, JT, JE, JA> PushedAuthorizationClient<C, JT, JE, JA>
where
    C: Profile,
    JT: JsonWebKeyType,
    JE: JweContentEncryptionAlgorithm<JT>,
    JA: JweKeyManagementAlgorithm + Clone,
{
    ...
    pub fn pushed_authorization_request<S, AD>(&self, state_fn: S) -> PushedAuthorizationRequest<AD>
    where
        S: FnOnce() -> CsrfToken,
        AD: AuthorizationDetaislProfile,
    {
        let inner = self.inner.authorize_url(state_fn);
        PushedAuthorizationRequest::new(inner, self.par_auth_url.clone(), vec![], None, None, None)
    }

    pub fn authorize_url(&self, request_uri: String) -> String {
